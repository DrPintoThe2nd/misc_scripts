...
rule mosdepth:
	...
	shell:
		"""
		MOSDEPTH_PRECISION=5 mosdepth -x -n -t6 -Q 3 -b 500000 {params.prefix} {input.bam}
		"""
...
#		query2 = os.path.join(cleaned_reads + "{sample}_R2.fastq.gz"),
	...
	shell:
		"""
		minimap2 -ax map-hifi -t6 {params.param} {input.target} {input.query1} | samtools sort -@6 -O bam -o {output.bam};
...
rule freebayes:
	...
	output:
		vcf = os.path.join(called_reads + "freebayes_calls.vcf"),
	shell:
		"""
		freebayes -f {input.target} -g 200 -L {input.bam_list} > {output.vcf}
		"""
